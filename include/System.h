...
public:
  ...
  // Returns true if there has been a reset since the last call to this function
  bool HasReset();
  ...
  // Save camera trajectory in the TUM format, but first transforming it into the target frame
  // Only for stereo and RGB-D. This method does not work for monocular.
  // Call first Shutdown()
  void SaveTrajectory(const string& filename, const Eigen::Matrix4f& T_target_source = Eigen::Matrix4f::Identity(),
                      const double start_timestamp = -1.0, const double end_timestamp = -1.0);
  ...
  void SaveKeyFrameTrajectory(const string& filename,
                              const Eigen::Matrix4f& T_target_source = Eigen::Matrix4f::Identity(),
                              const double start_timestamp = -1.0, const double end_timestamp = -1.0);

  // Save trajectory of specific maps
  void SaveTrajectory(const string& filename, Map* pMap);
  void SaveKeyFrameTrajectory(const string& filename, Map* pMap);

  void SaveAllMapTrajectories(const string& folder);
  ...
  std::vector<Eigen::Vector3f> GetFullPointCloud();

  std::vector<Eigen::Vector3f> GetTrackedPointCloud();
