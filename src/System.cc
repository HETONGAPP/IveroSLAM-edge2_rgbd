...
System::System(const string& strVocFile, const string& strSettingsFile, const eSensor sensor, const bool bUseViewer,
  ...
  cout << endl
       << "ORB-SLAM3 Copyright (C) 2017-2020 Carlos Campos, Richard Elvira, Juan J. Gómez, José M.M. Montiel and Juan "
  ...
  if (!node.empty() && node.isString() && node.string() == "1.0") {
    ...
    if (!node.empty() && node.isString()) { mStrLoadAtlasFromFile = (string)node; }

    node = fsSettings["System.SaveAtlasToFile"];
    if (!node.empty() && node.isString()) { mStrSaveAtlasToFile = (string)node; }
  }

  node = fsSettings["loopClosing"];
  bool activeLC = true;
  if (!node.empty()) { activeLC = static_cast<int>(fsSettings["loopClosing"]) != 0; }
  ...
  if (mSensor == IMU_STEREO || mSensor == IMU_MONOCULAR || mSensor == IMU_RGBD) mpAtlas->SetInertialSensor();
  ...
  mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer, mpAtlas, mpKeyFrameDatabase, strSettingsFile,
...
void System::SaveTrajectory(const string& filename, const Eigen::Matrix4f& T_target_source,
                            const double start_timestamp, const double end_timestamp) {
  Sophus::SE3f Ttc(T_target_source.block<3, 3>(0, 0), T_target_source.block<3, 1>(0, 3));
  ...
  for (list<Sophus::SE3f>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
    ...
    if (*lbL) continue;
    if (start_timestamp > 0 && *lT < start_timestamp) continue;
    if (end_timestamp > 0 && *lT > end_timestamp) continue;
    ...
    Sophus::SE3f Ttw = Ttc * Tcw;
    Sophus::SE3f Twt = Ttw.inverse();

    Eigen::Vector3f t = Twt.translation();
    Eigen::Quaternionf q = Twt.unit_quaternion();
    f << setprecision(9) << *lT << " " << setprecision(9) << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " "
      << q.y() << " " << q.z() << " " << q.w() << endl;
  }
  f.close();
}

void System::SaveKeyFrameTrajectory(const string& filename, const Eigen::Matrix4f& T_target_source,
                                    const double start_timestamp, const double end_timestamp) {
  Sophus::SE3f Ttc(T_target_source.block<3, 3>(0, 0), T_target_source.block<3, 1>(0, 3));
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (start_timestamp > 0 && pKF->mTimeStamp < start_timestamp) continue;
    if (end_timestamp > 0 && pKF->mTimeStamp > end_timestamp) continue;

    if (pKF->isBad()) continue;

    Sophus::SE3f Twc = pKF->GetPoseInverse();
    Sophus::SE3f Twt = Twc * Ttc.inverse();

    Eigen::Quaternionf q = Twt.unit_quaternion();
    Eigen::Vector3f t = Twt.translation();
    f << setprecision(9) << pKF->mTimeStamp << setprecision(7) << " " << t(0) << " " << t(1) << " " << t(2) << " "
...
void System::SaveTrajectory(const string& filename, Map* pMap) {
  cout << "Saving trajectory of map " << pMap->GetId() << " to " << filename << " ..." << endl;
  ...
  for (auto lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end(); lit != lend;
    ...
    if (!pKF || pKF->GetMap() != pMap) { continue; }
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      f << setprecision(9) << (*lT) << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " "
        << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    } else {
...
void System::SaveKeyFrameTrajectory(const string& filename, Map* pMap) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Quaternionf q = Twb.unit_quaternion();
      Eigen::Vector3f twb = Twb.translation();
      f << setprecision(9) << pKF->mTimeStamp << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2)
        << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
      ...
      f << setprecision(9) << pKF->mTimeStamp << " " << setprecision(9) << t(0) << " " << t(1) << " " << t(2) << " "
        << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
  }
  f.close();
...
void System::SaveAllMapTrajectories(const string& folder) {
  int i = 0;
  for (const auto& map : mpAtlas->GetAllMaps()) {
    std::string filename = folder + "/trajectory-map" + std::to_string(i) + ".txt";
    SaveTrajectory(filename, map);
    i++;
  }
}
...
void System::SaveDebugData(const int& initIdx) {
  ...
  SaveTrajectory("init_FrameTrajectoy_" + to_string(mpLocalMapper->mInitSect) + "_" + to_string(initIdx) + ".txt");
...
cv::Mat System::GetDrawnFrame() {
  return mpFrameDrawer->DrawFrame();
}

std::vector<Eigen::Vector3f> System::GetFullPointCloud() {
  const auto map_points = mpAtlas->GetAllMapPoints();
  std::vector<Eigen::Vector3f> output_cloud;
  for (const auto& mp : map_points) { output_cloud.push_back(mp->GetWorldPos()); }
  return output_cloud;
}

std::vector<Eigen::Vector3f> System::GetTrackedPointCloud() {
  std::vector<Eigen::Vector3f> output_cloud;
  auto tracked_points = GetTrackedMapPoints();
  for (auto mp : tracked_points) {
    if (mp) { output_cloud.push_back(mp->GetWorldPos()); }
  }
  return output_cloud;
}
...
string System::CalculateCheckSum(string filename, int type) {
  ...
  MD5_Init(&md5Context);
  while (int count = f.readsome(buffer, sizeof(buffer))) { MD5_Update(&md5Context, buffer, count); }
