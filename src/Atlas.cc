...
void Atlas::ChangeMap(Map* pMap) {
  ...
  if (mpCurrentMap) { mpCurrentMap->SetStoredMap(); }
...
void Atlas::PostLoad() {
  ...
  for (GeometricCamera* pCam : mvpCameras) { mpCams[pCam->GetId()] = pCam; }
...
long unsigned int Atlas::GetNumLivedKF() {
  ...
  for (Map* pMap_i : mspMaps) { num += pMap_i->GetAllKeyFrames().size(); }
...
long unsigned int Atlas::GetNumLivedMP() {
  unique_lock<mutex> lock(mMutexAtlas);
  long unsigned int num = 0;
  for (Map* pMap_i : mspMaps) { num += pMap_i->GetAllMapPoints().size(); }
...
map<long unsigned int, KeyFrame*> Atlas::GetAtlasKeyframes() {
  ...
  for (Map* pMap_i : mvpBackupMaps) {
    ...
    for (KeyFrame* pKF_j_Mi : vpKFs_Mi) { mpIdKFs[pKF_j_Mi->mnId] = pKF_j_Mi; }
  }
