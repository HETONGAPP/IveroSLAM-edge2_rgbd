...
Frame::Frame(const Frame& frame)
  ...
  if (frame.HasVelocity()) { SetVelocity(frame.GetVelocity()); }
...
Frame::Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const double& timeStamp, ORBextractor* extractorLeft,
  ...
  mTimeORB_Ext =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndExtORB - time_StartExtORB).count();
#endif
...
void Frame::AssignFeaturesToGrid() {
  ...
  for (unsigned int i = 0; i < FRAME_GRID_COLS; i++)
    for (unsigned int j = 0; j < FRAME_GRID_ROWS; j++) {
      ...
      if (Nleft != -1) { mGridRight[i][j].reserve(nReserve); }
    }
