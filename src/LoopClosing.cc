...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  if (mnLoopNumCoincidences > 0) {
    ...
    bool bCommonRegion =
        DetectAndReffineSim3FromLastKF(mpCurrentKF, mpLoopMatchedKF, gScw, numProjMatches, mvpLoopMPs, vpMatchedMPs);
    if (bCommonRegion) {
      ...
      if (!mbLoopDetected) { cout << "PR: Loop detected with Reffine Sim3" << endl; }
    } else {
  ...
  if (mnMergeNumCoincidences > 0) {
    ...
    vector<MapPoint*> vpMatchedMPs;
    bool bCommonRegion =
        DetectAndReffineSim3FromLastKF(mpCurrentKF, mpMergeMatchedKF, gScw, numProjMatches, mvpMergeMPs, vpMatchedMPs);
    if (bCommonRegion) {
  ...
  if (!bMergeDetectedInKF || !bLoopDetectedInKF) {
    ...
    double timeDataQuery =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndQuery - time_StartQuery).count();
  ...
  if (!bMergeDetectedInKF && !vpMergeBowCand.empty()) {
    mbMergeDetected = DetectCommonRegionsFromBoW(vpMergeBowCand, mpMergeMatchedKF, mpMergeLastCurrentKF, mg2oMergeSlw,
  ...
  if (mbMergeDetected || mbLoopDetected) { return true; }
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, g2o::Sim3& gScw,
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    if (mpTracker->mSensor == System::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2()) bFixedScale = false;
    int numOptMatches =
        Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale, mHessian7x7, true);

    // Verbose::PrintMess("Sim3 reffine: There are " + to_string(numOptMatches) + " matches after of the optimization ",
    // Verbose::VERBOSITY_DEBUG);
  ...
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
    {
      ...
      if (bConverge) {
        ...
        // cout <<"BoW: " << numProjMatches << " matches between " << vpMapPoints.size() << " points with coarse Sim3"
        // << endl;
...
bool LoopClosing::DetectCommonRegionsFromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, g2o::Sim3& gScw,
  ...
  int nProjMatches = 30;
  if (nNumProjMatches >= nProjMatches) { return true; }
...
void LoopClosing::CorrectLoop() {
...

  {
    ...
    // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align with the other side of the
    // loop
  ...
  double timeFusion =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndFusion - time_StartFusion).count();
...
void LoopClosing::MergeLocal() {
  ...
  // Relationship to rebuild the essential graph, it is used two times, first in the local window and later in the rest
  // of the map
  KeyFrame* pNewChild;
  KeyFrame* pNewParent;
...

  {
    ...
    for (KeyFrame* pKFi : spLocalWindowKFs) {
      ...
      if (pCurrentMap->isImuInitialized()) { pKFi->SetVelocity(pKFi->mVwbMerge); }
    }
  ...
  if (vpCurrentMapKFs.size() == 0) {
    ...
    if (mpTracker->mSensor != System::MONOCULAR) {
      Optimizer::OptimizeEssentialGraph(mpCurrentKF, vpMergeConnectedKFs, vpLocalCurrentWindowKFs, vpCurrentMapKFs,
                                        vpCurrentMapMPs);
    }
  ...
  double timeOptEss =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndOptEss - time_EndWeldingBA).count();
...
void LoopClosing::MergeLocal2() {
  ...
  vector<MapPoint*> vpCheckFuseMapPoint; // MapPoint vector from current map to allow to fuse duplicated points with the
                                         // old map (merge)
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF) {
  ...
  // We need to propagate the correction through the spanning tree
  {
    ...
    if (!mbStopGBA) {
      ...
      while (!mpLocalMapper->isStopped() && !mpLocalMapper->isFinished()) { usleep(1000); }
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        if(dist > 1)
        {
            cout << "--To much distance correction: It has " << pKF->GetConnectedKeyFrames().size() << " connected KFs"
        << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(80).size() << " connected KF with 80 common matches
        or more" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(50).size() << " connected KF with 50 common
        matches or more" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(20).size() << " connected KF with 20
        common matches or more" << endl;
      ...
      double timeFGBA =
          std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndUpdateMap - time_StartFGBA)
              .count();
