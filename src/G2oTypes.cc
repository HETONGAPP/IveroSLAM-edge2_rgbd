...
bool VertexPose::read(std::istream& is) {
  ...
  for (int idx = 0; idx < num_cams; idx++) {
    ...
    for (int i = 0; i < 3; i++) { is >> tcw[idx](i); }

    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) is >> Rbc[idx](i, j);
    }
    for (int i = 0; i < 3; i++) { is >> tbc[idx](i); }
...
bool VertexPose::write(std::ostream& os) const {
  ...
  for (int idx = 0; idx < num_cams; idx++) {
    ...
    for (int i = 0; i < 3; i++) { os << tcw[idx](i) << " "; }

    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) os << Rbc[idx](i, j) << " ";
    }
    for (int i = 0; i < 3; i++) { os << tbc[idx](i) << " "; }
...
void EdgeInertial::computeError() {
  ...
  const IMU::Bias b1(VA1->estimate()[0], VA1->estimate()[1], VA1->estimate()[2], VG1->estimate()[0], VG1->estimate()[1],
                     VG1->estimate()[2]);
