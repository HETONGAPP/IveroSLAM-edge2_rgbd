...
template <>
int Settings::readParameter<int>(cv::FileStorage& fSettings, const std::string& name, bool& found,
                                 const bool required) {
  cv::FileNode node = fSettings[name];
  if (node.empty()) {
    if (required) {
...
ostream& operator<<(std::ostream& output, const Settings& settings) {
  ...
  if (!settings.vPinHoleDistorsion1_.empty()) {
    ...
    for (float d : settings.vPinHoleDistorsion1_) { output << " " << d; }
    output << " ]" << endl;
  }
  ...
  if (settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO) {
    ...
    if (!settings.vPinHoleDistorsion2_.empty()) {
      output << "\t-Camera 1 distortion parameters: [ ";
      for (float d : settings.vPinHoleDistorsion2_) { output << " " << d; }
      output << " ]" << endl;
    }
  }
